    ...
    if (!data.statusCode || data.statusCode >= 400 || data.statusCode < 200) {
      ...
      if (!data || !(data instanceof HttpException)) {
        if (data && (data as any).message) {
          exception = new InternalServerException((data as any).message);
        } else {
      ...
      if (
        !BLACKLIST_LOG.includes(request.originalUrl) &&
        !request.originalUrl.startsWith(SWAGGER_PATH)
      ) {
        Logger.info(
          `<-- ${code} Method: ${request.method}, URL: ${request.originalUrl}`
        );
        Logger.info(
          ` Data : ${JSON.stringify({
            code,
      ...
      if (data instanceof SuccessResponse) {
        if (
          ...
          Logger.info(
            `<-- ${data.statusCode} Method: ${request.method}, URL: ${request.originalUrl}`
          );
          Logger.info(
