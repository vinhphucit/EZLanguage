...
export class UserController {
    ...
    public async get(req: Request, res: Response, next: NextFunction) {
        try {            
            ...
            next(new SuccessResponse(new GetUsersResponse(result.items.map(value => new GetUserByIdResponse(value)), result.start, result.limit, result.totalItems, result.sort, result.query)))
        } catch (e) {
            return next(e)
        }
    }
